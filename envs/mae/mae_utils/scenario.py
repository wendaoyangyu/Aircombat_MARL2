...
import numpy as np
from envs.mae.mae_utils.core import Entity
...
class BaseScenario:  
    ...
    def total_observation(self, env):
        """
        Total observation for close nvn.
        """
        states = tuple(
            self.observation(agent, env).astype(np.float32)
    ...
    def single_situation(simu_r: np.ndarray, simu_b: np.ndarray):
        ...
        # relative distance
        distance = np.linalg.norm(np.array([x_r, y_r, z_r]) - np.array([x_b, y_b, z_b]))
    ...
    def mini_max(
        ...
        action_b = rewards_b.index(min(rewards_b))
        simu_b_next = world.imitate_action(action_b, simu_b)
        for i in range(0, 7):
            action_r = i
