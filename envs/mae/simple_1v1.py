...
class raw_env(BaseEnv, EzPickle):
    ...
    """
    def __init__(self, N=1, max_cycles=200, continuous_actions=False,
    ...
    def stop_condition(self):
        # TODO: 存在重复获取终止奖励的情况
        r0 = self.world.agents[0] 
        b0 = self.world.scripts[0]
        steps = self.steps
        
        distance, delta_h, delta_v, q_r, q_b, beta_rb = \
            self.scenario.single_situation(r0.unpack, b0.unpack)
        ...
        if (100 <= distance <= 1000) and (q_r <= 30) and (q_b <= 60):  # q_b < 60
            ...
            r_end = -20
            win = -1
        elif steps == self.max_cycles:
            r_end = -20  # -20
            win = 0
        else:
            r_end = 0
        self.rewards['red_0'] += r_end
        if win != 2:
            self.terminations['red_0'] = True
            if win == 1:
                print("After the %d round, the red side won!" % self.steps)  # 应该考虑添加轮数的信息
            elif win == -1:
                print("After the %d round, the blue side won!" % self.steps)
...
class Scenario(BaseScenario):
    ...
    def total_observation(self, world: World):
        """
        Total observation for close 1v1.
        """
        r0 = world.agents[0]
        return self.observation(r0, world=world)
...
if __name__ == '__main__':
    ...
    test_api = False
    ac_mode = 1
    r_model = 0  # only start
    env = raw_env()
    env.reset()

    if ac_mode == 2:
        load_path = '../../runner/ppo/models/ad/scene2/ad_pb_300w.pkl'
        r_model = PPO.load(load_path)
    if ac_mode == 3:
        load_path = '../../runner/ppo/models/ac/scene2/ac_pb_80w.pkl'
        r_model = PPO.load(load_path)
        env = raw_env(continuous_actions=True)
        env.reset()
    ...
    if test_api is True:
        ...
        for agent in env.agent_iter():
            ...
            if ac_mode == 0:
                ...
                action, _ = r_model.predict(observation)
            elif ac_mode == 3:
                action, _ = r_model.predict(observation)
