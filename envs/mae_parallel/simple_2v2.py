...
"""
    ...
    4. CloseAircombat:
    - agent is_alive才会得到reward_function，否则reward=0
    - reward在env中进行了norm(对己方)
...
class raw_env(BaseEnv, EzPickle):
    ...
    """
    def __init__(self, N=2, max_cycles=200, continuous_actions=False, render_mode=None, scr_mode='mini_max'):
        EzPickle.__init__(
            self,
            N=N,
        ...
        self.shared_reward = True  # TODO: 如果有一agent dead，True的话还是会接收奖励
...
class Scenario(BaseScenario):
    ...
    def observation(self, agent: Agent, env: BaseEnv):
        ...
        # for other in world.entities:
            # if other is agent:
            #     continue
        for other in world.scripts:
            x_b, y_b, z_b, phi_b, mu_b, v_b = other.unpack
            distance, delta_h, delta_v, q_r, q_b, beta = self.single_situation(agent.unpack, other.unpack)
...
if __name__ == '__main__':
    ...
    red_mini_max = False
    if red_mini_max:
        env = raw_env(scr_mode='random')  # random
    else:
        env = raw_env(scr_mode='mini_max')  # mini_max

    done_n = [False] * env.num_agents
    actions = [2] * env.num_agents
    total_rewards = np.array([0] * env.num_agents, dtype=np.float64)
    reward_array = np.zeros((201, 2), dtype=float)
    ...
    if test_circle:
        ...
        while False in done_n:
            ...
            obs_n, reward_n, done_n, info_n = env.step(actions)
            total_rewards += np.array(reward_n)
            reward_array[env.steps-1] = reward_n
        print('total_rewards: ' + str(total_rewards))
        env.monitor()
