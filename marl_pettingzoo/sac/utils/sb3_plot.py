...
def plot_zh(data_1, only_fig: bool, path: str):
    ...
    one_reward_plot(average_reward_array_1, "MASAC", only_fig, path)
...
def one_reward_plot(reward_array: np.ndarray,
    ...
    plt.xlabel("Episodes", fontdict={"family": "Times New Roman", "size": 25})
    plt.ylabel("Mean_Accumulated_Reward", fontdict={"family": "Times New Roman", "size": 25})
...
def rate_plot_cn(
    ...
    plt.plot(win_array, label = "win_rate")
    plt.plot(tie_array, label = "tie_rate")
    plt.plot(lose_array, label = "lose_rate")
    plt.xticks(fontproperties='Times New Roman', size=20)
    plt.yticks(fontproperties='Times New Roman', size=20)
    # plt.xlabel("轮次", fontdict={"family": "SimSun", "size": 25})
    # plt.ylabel("胜负率", fontdict={"family": "SimSun", "size": 25})
    plt.xlabel("Episodes", fontdict={"family": "Times New Roman", "size": 25})
    plt.ylabel("Win/Loss Rate", fontdict={"family": "Times New Roman", "size": 25})
    plt.legend(prop={'family': 'Times New Roman', 'size': 22})
    plt.grid()
    plt.margins(x=0)
